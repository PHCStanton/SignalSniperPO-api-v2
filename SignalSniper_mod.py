...
class ModularSelfBot:
    ...
    async def access_channel(self, channel_config):
        """Access a Telegram channel or bot using the channel configuration."""
        ...
        source_type = getattr(channel_config, 'source_type', None) or channel_config.config.get('source_type', 'telegram_channel')
        
        logger.info(f"Accessing {source_type}: {channel_name}")
        
        try:
            # Handle telegram bots differently from channels
            if source_type == "telegram_bot":
                # For bots, use the bot username directly
                if channel_id and channel_id.startswith("@"):
                    ...
                    logger.error(f"Invalid bot channel_id format: {channel_id}. Expected @username format.")
                    return None
            
            # Handle regular channels (existing logic)
            else:
                # Try to use channel ID if available
                if channel_id:
                    try:
                        # For channels, try numeric ID first
            ...
            logger.error(f"Error accessing {source_type} {channel_name}: {str(e)}")
            return None
    ...
    def validate_timeframe(self, signal: Dict) -> Tuple[bool, str]:
        """Validate if the signal's timeframe is enabled with channel-specific support."""
        try:
            ...
            if not os.path.exists(timeframe_config_path):
                # If no timeframe config exists, allow all timeframes
            ...
            # Check if timeframe is suspended globally
            suspended_timeframes = timeframe_control.get("suspended_timeframes", [])
            if signal_timeframe in suspended_timeframes:
                return False, f"Timeframe {signal_timeframe} minutes is globally suspended"
            
            # Get channel information for channel-specific filtering
            channel_name = signal.get("channel", "")
            parser_type = signal.get("parser_type", "")
            
            # Check for channel-specific timeframe configuration
            channel_specific = timeframe_config.get("channel_specific", {})
            channel_config = None
            
            # Try to find channel-specific config by parser type or channel name
            for config_key, config_data in channel_specific.items():
                if (config_key.lower() in parser_type.lower() or 
                    config_key.lower() in channel_name.lower().replace(" ", "_")):
                    channel_config = config_data
                    logger.info(f"🎯 TIMEFRAME: Using channel-specific config for {config_key}")
                    break
            
            # If channel-specific config found, use it
            if channel_config:
                # Check if channel overrides global settings
                if not channel_config.get("override_global", False):
                    # Channel doesn't override global, check global settings first
                    ...
                    if timeframe_str in timeframe_settings:
                        global_enabled = timeframe_settings[timeframe_str].get("enabled", True)
                        if not global_enabled:
                            return False, f"Timeframe {signal_timeframe} minutes is globally disabled"
                
                # Check channel-specific timeframe settings
                channel_timeframes = channel_config.get("timeframes", {})
                timeframe_str = str(signal_timeframe)
                
                if timeframe_str in channel_timeframes:
                    channel_enabled = channel_timeframes[timeframe_str].get("enabled", True)
                    if not channel_enabled:
                        return False, f"Timeframe {signal_timeframe} minutes is disabled for {parser_type}"
                    
                    # Log priority information if available
                    priority = channel_timeframes[timeframe_str].get("priority", "normal")
                    logger.info(f"🎯 TIMEFRAME: {signal_timeframe}min signal accepted (priority: {priority})")
                    return True, f"Timeframe {signal_timeframe} minutes is enabled for {parser_type} (priority: {priority})"
                else:
                    # Timeframe not specifically configured for channel, check filtering rules
                    filtering_rules = channel_config.get("filtering_rules", {})
                    if filtering_rules.get("skip_disabled_timeframes", True):
                        return False, f"Timeframe {signal_timeframe} minutes not configured for {parser_type}"
            else:
                # No channel-specific config, use global settings
                timeframe_settings = timeframe_config.get("timeframe_settings", {})
                timeframe_str = str(signal_timeframe)
            ...
            logger.error(f"Error validating timeframe: {str(e)}")
            # If there's an error, allow the signal to proceed
    ...
    def execute_trade_threaded(self, signal: Dict, timestamp_record: Dict = None) -> None:
        ...
        try:
            # Check if time offset should be applied
            time_offset_config = self.config.get("time_offset", {})
            if time_offset_config.get("enabled", False):
                # Get channel-specific offset configuration
                channel_name = signal.get("channel", "").lower().replace(" ", "_")
                parser_type = signal.get("parser_type", "").lower()
                signal_duration = signal.get("expiry", 0)  # Duration in minutes
                ...
                if channel_offset_config and channel_offset_config.get("enabled", False):
                    ...
                    if signal_duration in apply_to_durations:
                        offset_value = channel_offset_config.get("offset_value", 0)
                        if offset_value > 0:
                            logger.info(f"⏱️ TIME OFFSET: Applying {offset_value}-second delay for {signal_duration}-minute {channel_name} signal")
                            time.sleep(offset_value)
                        else:
                            logger.debug(f"Time offset is 0 for {channel_name}, proceeding immediately")
                    else:
                        logger.debug(f"Duration {signal_duration} not in apply_to_durations {apply_to_durations} for {channel_name}")
                else:
                    logger.debug(f"Time offset not enabled for channel: {channel_name}")
