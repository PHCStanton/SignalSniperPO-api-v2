...
## Pocket Option API Reference

For future reference, here are the key parameters for the Pocket Option API:

- **asset**: The currency pair, e.g., "EURUSD", "NZDUSD_otc"
- **amount**: The trade amount
- **dir**: Direction, "call" for BUY or "put" for SELL
- **duration**: Trade duration in seconds, commonly 30, 60, or 120

### Example Code Format

```python
if check_connect:
    account.change_balance("PRACTICE")  # or "REAL" for live trading
    asset = "EURUSD"
    amount = 1
    dir = "call"  # "call" for BUY, "put" for SELL
    duration = 30  # Duration in seconds
    buy_info = account.buy(asset, amount, dir, duration)
    print("Trade result:", account.check_win(buy_info["id"]))
    account.close()
```

For more details, refer to the PocketOption API documentation: https://lu-yi-hsun.github.io/pocketoptionapi/
