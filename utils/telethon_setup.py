#!/usr/bin/env python3
"""
telethon_setup.py - Setup and test Telethon connection for the Pocket Option trading bot.

This script helps set up and test the Telethon connection for monitoring the
"BINARY TRADING CLUB" channel for trading signals. It uses a user account
to access the channel discreetly and includes functionality to test connectivity
and parse signals.

Usage:
    python telethon_setup.py --api-id YOUR_API_ID --api-hash YOUR_API_HASH
    python telethon_setup.py --config ../config/telegram_config.json
    python telethon_setup.py --env  # Load from .env file
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("telethon_setup.log")
    ]
)
...
class TelethonSetup:
    ...
    def _load_config(self) -> Dict:
        ...
        return {
            ...
            "monitoring": {
                ...
                "save_messages": True,
                "messages_file": "data/telegram_messages.json",
                "max_messages": 1000
            }
        }
    ...
    async def initialize(self) -> None:
        """
        Initialize the Telegram client with improved error handling and session reuse.
    ...
    async def find_channel(self, channel_name: Optional[str] = None) -> Optional[Channel]:
        ...
        async for dialog in self.client.iter_dialogs():
            if dialog.name == channel_name or (hasattr(dialog.entity, 'username') and dialog.entity.username == channel_name):
...
async def main():
    parser = argparse.ArgumentParser(description='Set up Telethon for Pocket Option trading bot')
    parser.add_argument('--api-id', type=int, help='Telegram API ID')
    parser.add_argument('--api-hash', type=str, help='Telegram API hash')
    parser.add_argument('--session', type=str, default='pocket_option_userbot', help='Session name')
    parser.add_argument('--config', type=str, default='config/telegram_config.json', help='Configuration file path')
    ...
    if args.env:
        ...
        if api_id:
            try:
                ...
                print(f"Error: TELEGRAM_API_ID must be an integer, got: {api_id}")
                sys.exit(1)
    ...
    try:
        # Initialize client
        success = await setup.initialize()
        if not success:
        ...
        if setup.channel:
            ...
            if connected:
                ...
                print(f"  Username: {setup.config.get('channel_username', 'N/A')}")
                ...
                if args.monitor:
                    ...
                    monitor = input("Would you like to monitor the channel for signals? (y/n): ")
                    if monitor.lower() == 'y':
                        duration = input("Enter monitoring duration in seconds (default: 300): ")
                        try:
                            duration = int(duration)
                        except ValueError:
                            duration = 300
                        await setup.run_signal_monitor(duration)
            else:
...
if __name__ == "__main__":
    print("Pocket Option Trading Bot - Telethon Setup")
    print("------------------------------------------")
    print("This script helps you set up and test Telethon for monitoring the")
    print("BINARY TRADING CLUB channel for trading signals.")
    print("")
